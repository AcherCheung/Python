...
def getLocationBy_lat_lng(lat, lng):
    ...
    """
    secret_key = 'pYfpgPARZdpGbP5MmakFkEIjHX499a1a'  # 请修改这里
    ...
    # 只展示商业区
    formatted_address = baidu_map_address["result"]["business"]
...
def getLocation(filename):
    ...
    if data:  # 取得照片的拍摄位置
        try:
            ...
            pass
    # 只取地址的前两个详细地址，一般是返回三个，有点长
    locationStr = locationStr.split(',')
    locationStr = locationStr[1:]
    locationStr = (',').join(x for x in locationStr)
    print(locationStr)
    return locationStr
...
def add_watermark(filename, location, photoTime):
    ...
    date_size = int(0.03 * width)  # 时间字体大小
    addr_size = int(0.02 * width)  # 地址字体大小
    cur_path = os.path.abspath(__file__)
    parent_path = os.path.abspath(os.path.dirname(cur_path) + os.path.sep + ".")
    fontfile_datepath = parent_path + '\mtc7seg.ttf'
    fontfile_addrpath = parent_path + '\Arial-Unicode-MS.ttf'
    myfont_date = ImageFont.truetype(fontfile_datepath, size=date_size)  # 80, 4032*3024
    myfont_addr = ImageFont.truetype(fontfile_addrpath, size=addr_size)  # 80, 4032*3024
    ...
    if width > height:
        ...
        d_width, d_height = 0.74 * width, 0.85 * height  # 字体的相对位置（胶卷位置）
        draw.text((d_width, d_height), location, font=myfont_addr, fill=fillcolor)  # (-1200, -320)
        ...
        # 参数一：位置（x轴，y轴）；参数二：填写内容；参数三：字体；参数四：颜色
        d_width, d_height = 0.75 * width, 0.905 * height  # 字体的相对位置（胶卷位置）
        draw.text((d_width, d_height), location, font=myfont_addr, fill=fillcolor)  # (-1200, -320)
    ...
    for obj in os.listdir(os.curdir):
        if os.path.isfile(obj):
            if isTargetedFileType(obj):  # 对满足过滤条件的文件，加时间和地点水印
                ...
                add_watermark(obj, location, photoTime)  # 加时间和地点水印
                # add_watermark(obj, photoTime)  # 加时间


'''=======================GUI界面生成start==================='''
frameT = Tk()
frameT.geometry('550x160+400+200')
frameT.title('给文件夹内的照片添加拍摄日期')
frame = Frame(frameT)
...
def run():
    path = v.get()  # 照片位置
    scandir(path)
    tkinter.messagebox.showinfo(title='提示', message='处理完毕！请查看此文件夹下的watermark文件夹~')

btn = Button(frame, width=20, text='选择文件夹', font=("宋体", 14), command=fileopen).pack(fil=X, padx=20)
