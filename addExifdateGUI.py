...
# 判断是否是支持的文件类型
SUFFIX_FILTER = ['.jpg', '.png', '.mpg', '.bmp', '.jpeg']  # 仅测试了jpeg，jpg
...
def getPhotoTime(filename):
    ...
    try:
        if os.path.isfile(filename):
            fd = open(filename, 'rb')
            # fd_row = open(filename, 'rb')
            # fd1 = binascii.b2a_hex(fd_row.encode('utf-16-be'))
            # fd = binascii.a2b_hex(fd1).decode('utf-16-be')
        else:
